...
public class AvaliacaoComentarioDAO {
    ...
    public AvaliacaoComentarioDAO() {
        try {
            ...
            insertAvaliacaoComentarioStatement = AvaliacaoComentarioDAO.conexao.prepareStatement(
                    "INSERT INTO avaliacao_comentario(avaliacao, id_comentario_avaliado, id_usuario_proprietario) "
                    + "VALUES (?, ?, ?)", Statement.RETURN_GENERATED_KEYS);

            selectAvaliacaoByIdComentarioAndByIdUsuarioStatement = AvaliacaoComentarioDAO.conexao.prepareStatement(
                    "SELECT * "
                    + "FROM avaliacao_comentario "
                    + "WHERE id_comentario_avaliado = ? AND id_usuario_proprietario = ?", Statement.RETURN_GENERATED_KEYS);
            selectAllAvaliacoesByIdComentarioAndByIdUsuarioStatement = AvaliacaoComentarioDAO.conexao.prepareStatement(
                    "SELECT * "
                    + "FROM avaliacao_comentario "
                    + "WHERE id_comentario_avaliado = ?", Statement.RETURN_GENERATED_KEYS);
            selectCountAvaliacoesPositivasComentariosByIdUsuarioStatement = AvaliacaoComentarioDAO.conexao.prepareStatement(
                    "SELECT COUNT(*) AS qtd_avaliacoes_positivas "
                    + "FROM avaliacao_comentario "
                    + "WHERE id_usuario_proprietario = ? AND avaliacao > 0", Statement.RETURN_GENERATED_KEYS);
            selectCountAvaliacoesNegativasComentariosByIdUsuarioStatement = AvaliacaoComentarioDAO.conexao.prepareStatement(
                    "SELECT COUNT(*) AS qtd_avaliacoes_negativas "
                    + "FROM avaliacao_comentario "
                    + "WHERE id_usuario_proprietario = ? AND avaliacao < 0", Statement.RETURN_GENERATED_KEYS);
            selectCountAvaliacoesPositivasMeusComentariosByIdUsuarioStatement = AvaliacaoComentarioDAO.conexao.prepareStatement(
                    "SELECT COUNT(*) AS qtd_avaliacoes_positivas "
                    + "FROM avaliacao_comentario AS ac, comentario AS co "
                    + "WHERE co.id_usuario_proprietario = ? AND co.id_comentario = ac.id_comentario_avaliado AND ac.avaliacao > 0", Statement.RETURN_GENERATED_KEYS);
            selectCountAvaliacoesNegativasMeusComentariosByIdUsuarioStatement = AvaliacaoComentarioDAO.conexao.prepareStatement(
                    "SELECT COUNT(*) AS qtd_avaliacoes_negativas "
                    + "FROM avaliacao_comentario AS ac, comentario AS co "
                    + "WHERE co.id_usuario_proprietario = ? AND co.id_comentario = ac.id_comentario_avaliado  AND ac.avaliacao < 0", Statement.RETURN_GENERATED_KEYS);

            updateAvaliacaoComentarioByIdComentarioAndIdUsuarioStatement = AvaliacaoComentarioDAO.conexao.prepareStatement(
                    "UPDATE avaliacao_comentario "
                    + "SET avaliacao = ? "
                    + "WHERE id_comentario_avaliado = ? AND id_usuario_proprietario = ?", Statement.RETURN_GENERATED_KEYS);

            deleteAvaliacaoComentarioByIdComentarioAndIdUsuarioStatement = AvaliacaoComentarioDAO.conexao.prepareStatement(
